...
class Replay:
    ...
    def parse_replay(filepath):
        ...
        command = f"{config.PATH} -map \"{replay_filepath}\""
    ...
    def get_player_from_replay(match_as_json, index):
        ...
        return Player(name, race, apm, eapm, stats, player_id)
    ...
    def get_match_from_replay(match_as_json, players):
        ...
        winner_id = match_as_json['Computed']['WinnerTeam']
        print(f"Winner: {winner_id}")
        match_winner = Replay.get_winner_from_id(players, winner_id)

        return Match(map_name, map_description, players, match_type, match_date, match_winner)
    ...
    '''
    @staticmethod
    def get_winner_from_id(players, winner_id):
        winner = "Loss"
        for player in players:
            if player.stats['Team'] == winner_id:
                winner = "Win"
        return winner
