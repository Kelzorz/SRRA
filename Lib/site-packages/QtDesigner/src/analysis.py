class MatchHistory:
    ...
    def get_list_of_apms(self):
        return [match.players[0].apm for match in self.match_history]
...
class WinRatio:
    ...
    def set_win_ratio_from_match_history(self, match_history):
        for match in match_history:
            if match.players[0].match_outcome == "Win":  # if our player won
                self.wins += 1
            self.matches += 1
...
class MatchUps:
    ...
    def match_outcomes(self, match_history):
        for match in match_history:
            ...
            if (match.players[0].match_outcome == "Win"):
