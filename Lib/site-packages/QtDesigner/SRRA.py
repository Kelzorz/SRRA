            ...
            for subdir, dirs, files in os.walk(AUTOSAVE_LOCATION):
                for file in files:
                    ...
                    if filepath.endswith(".rep"):
                        try:
                            match_as_json = players.Match.parseReplay(filepath)
                            print(match_as_json)
                            # checks if this is a 1v1 match
                            if players.Match.isOneVSOne(match_as_json):

                                my_index, index = players.Match.getPlayerIndex(
                                    match_as_json, YOUR_LADDER_NAMES)

                                if match_as_json['Header']['Players'][my_index]['Name'] in YOUR_LADDER_NAMES:

                                    you = players.Replay.getPlayerFromReplay(
                                        match_as_json, my_index)
                                    opponent = players.Replay.getPlayerFromReplay(
                                        match_as_json, index)
                                    ...
                                    # statistics
                                    current_match = players.Replay.getMatchFromReplay(
                                        match_as_json)
                                    matches += 1
